        ...
        for batch_size in 1000 3000 5000 10000; do
            for seed in $(seq 1 5); do
                python cs285/scripts/run_hw2.py --env_name InvertedPendulum-v4 -n 100 \
                    --exp_name pendulum_batchsz{$batch_size}_s{$seed}_discnt{$discount}_nlayers{$n_layers} \
                    -rtg --use_baseline -na \
            ...
            for lambda in 0.95 0.99; do
                for seed in $(seq 1 5); do
                    python cs285/scripts/run_hw2.py --env_name InvertedPendulum-v4 -n 100 \
                        --exp_name pendulum_batchsz{$batch_size}_s{$seed}_discnt{$discount}_nlayers{$n_layers}_gaelambda{$lambda} \
                        -rtg --use_baseline -na \
                        --batch_size $batch_size \
                        --seed $seed \
                        --discount $discount \
