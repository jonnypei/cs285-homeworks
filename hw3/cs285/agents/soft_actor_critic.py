...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount * torch.where(done, 0, 1) * next_qs
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(torch.stack([obs] * self.num_actor_samples), action)
    ...
    def update(
        ...
        if self.target_update_period and step % self.target_update_period == 0: # hard update
            self.update_target_critic()
        elif self.soft_target_update_rate: # soft update
