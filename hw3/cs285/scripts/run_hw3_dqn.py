...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        next_obs_for_insertion = next_observation[-1] if next_observation.ndim > 2 else next_observation
        obs_for_insertion = observation[-1] if observation.ndim > 2 else observation
        done_for_insertion = done and not truncated
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(
                action=action,
                reward=reward,
                next_observation=next_obs_for_insertion,
                done=done_for_insertion
            )
        else:
            # We're using the regular replay buffer
            replay_buffer.insert(
                observation=obs_for_insertion,
                action=action,
                reward=reward,
                next_observation=next_obs_for_insertion,
                done=done_for_insertion
