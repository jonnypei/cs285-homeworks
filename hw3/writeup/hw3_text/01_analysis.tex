...
\def\answer{2} % <--- TODO: insert index (1/2) of answer
...
\def\answer{4} % <--- TODO: insert index of answer(s)
\ifsolutions\solve\thesubsection\fi
\begin{itemize}
...
\begin{sol}
	We are given 
	\[\phi_{k+1} \gets \underset{\phi\in\Phi}{\arg\min}  \sum_{j,t} \bigl( y_{j,t}-Q_{\phi}(\mathbf s_{j,t},\mathbf a_{j,t}) \bigr)^2.\]
	Since we have another policy $\pi'(\mathbf{a}_t \mid \mathbf{s}_t)$ whose rollouts are included in training data $\mathcal{D}$, we can rewrite the importance sampling weight $w_{j, k}$ as follows:
	\[w_{j, t} = \prod_{t' = t}^{t + N - 1} \frac{\pi(\mathbf{a}_{j, t'} \mid \mathbf s_{j, t'})}{\pi' (\mathbf a_{j, t'} \mid \mathbf s_{j, t'})}\]
	for all $j, t$. Note that we don't need to change \eqref{eq:q_update} when $N=1$, but we do need to when $N \rightarrow \infty$. Thus, we can rewrite Eq. (\ref{eq:q_update}) as 
	\begin{align*}
		\phi_{k+1} &\gets \underset{\phi\in\Phi}{\arg\min} \sum_{j,t} w_{j, t}\bigl( y_{j,t}-Q_{\phi}(\mathbf s_{j,t},\mathbf a_{j,t}) \bigr)^2 \\
		&= \underset{\phi\in\Phi}{\arg\min} \sum_{j,t} \left[\prod_{t' = t}^{t + N - 1} \frac{\pi(\mathbf{a}_{j, t'} \mid \mathbf s_{j, t'})}{\pi' (\mathbf a_{j, t'} \mid \mathbf s_{j, t'})} \right]\bigl( y_{j,t}-Q_{\phi}(\mathbf s_{j,t},\mathbf a_{j,t}) \bigr)^2
	\end{align*}
\end{sol}
