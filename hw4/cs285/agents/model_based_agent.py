...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        input_normalized = (torch.cat([obs, acs], dim=1) - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        pred_obs_delta = self.dynamics_models[i](input_normalized)
        
        obs_delta = next_obs - obs
        obs_delta_normalized = (obs_delta - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)
        
        loss = self.loss_fn(pred_obs_delta, obs_delta_normalized)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_acs_mean = torch.mean(input, dim=0)
        self.obs_acs_std = torch.std(input, dim=0)
    ...
    def get_dynamics_predictions(
        ...
        input_normalized = (torch.cat([obs, acs], dim=1) - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        pred_obs_delta = self.dynamics_models[i](input_normalized)
        pred_next_obs = obs + self.obs_delta_mean + pred_obs_delta * self.obs_delta_std
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                if i == 0:
                    top_indices = np.argsort(self.evaluate_action_sequences(obs, action_sequences))[-self.cem_num_elites:]
                    top_action_sequences = action_sequences[top_indices]
                    
                    elite_mean = np.mean(top_action_sequences, axis=0)
                    elite_std = np.std(top_action_sequences, axis=0)
                else:
                    ...
                    elite_mean = self.cem_alpha * np.mean(top_action_sequences, axis=0) + (1 - self.cem_alpha) * elite_mean
                    elite_std = self.cem_alpha * np.std(top_action_sequences, axis=0) + (1 - self.cem_alpha) * elite_std
            ...
            return action_sequences[np.argmax(rewards)][0]
        else:
