...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        preds_ensembled = [mb_agent.get_dynamics_predictions(i, 
                                                             np.expand_dims(ob, axis=0), 
                                                             np.expand_dims(ac, axis=0)
                                                             ) 
                           for i in range(mb_agent.ensemble_size)]
        next_ob = np.mean(preds_ensembled, axis=0).squeeze()
    ...
    for itr in range(config["num_iters"]):
        ...
        if sac_config is not None:
            ...
            for i in tqdm.trange(
                ...
                # train SAC
                batch = ptu.from_numpy(sac_replay_buffer.sample(sac_config["batch_size"]))
                sac_agent.update(
                    batch["observations"],
                    batch["actions"],
                    batch["rewards"],
