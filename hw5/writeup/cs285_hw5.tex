...
\begin{figure}[!ht]\color{darkblue}
    \begin{center}
    \includegraphics[width=2.2in]{../exploration_visualization/PointmassEasy-v0_random.png}
    \includegraphics[width=2.2in]{../exploration_visualization/PointmassMedium-v0_random.png}
    \includegraphics[width=2.2in]{../exploration_visualization/PointmassHard-v0_random.png}    
\end{center}
...
\begin{sol} I provide the final dataset visualizations for each of the easy, medium, and hard environments below:
    \begin{figure}[!ht]\color{darkblue}
        \begin{center}
        \includegraphics[width=2.25in]{../exploration_visualization/PointmassEasy-v0_rnd1.0.png}
        \includegraphics[width=2.25in]{../exploration_visualization/PointmassMedium-v0_rnd1.0.png}
        \includegraphics[width=2.25in]{../exploration_visualization/PointmassHard-v0_rnd1.0.png}    
    \end{center}
        \caption{Dataset Visualization of Easy (left), Medium (center), and Hard (right) Environments}
    \end{figure}
    \end{sol}
...
\begin{sol}
    After implementing the CQL agent, we ablate the $\alpha$ parameter, varying across the values $\{0, 0.01, 0.1, 1, 10\}$. We plot the $Q$-values and eval returns below:
    \begin{figure}[!ht]\color{darkblue}
        \begin{center}
        \includegraphics[width=3in]{q4.1_qvals.PNG}
        \includegraphics[width=3in]{q4.1_return.PNG}
        \end{center}
        \caption{$Q$-values (left) and Evaluation Returns (right) on the \texttt{PointmassMedium-v0} environment.}
    \end{figure} 
    We see that generally, smaller $\alpha$ values (e.g. near 0.01) result in better overall performance. This is because the agent is able to learn a good policy when the penalty is small. The large penalty ($\alpha \geq 1$) results in relatively flat return curves, as the heavy penalty hinders convergence.
\end{sol}
...
\begin{sol}
    After implementing the IQL and AWAC agents, we compare their evaluation returns on the \texttt{PointmassEasy-v0} and \texttt{PointmassMedium-v0} environments below:
    \begin{figure}[!ht] \color{darkblue}
        \begin{center}
        \includegraphics[width=3in]{q4.2_easy.PNG}
        \includegraphics[width=3in]{q4.2_medium.PNG}
        \end{center}
        \caption{Comparing IQL and AWAC on the \texttt{PointmassEasy-v0} (left) and \texttt{PointmassMedium-v0} (right) environments.}
    \end{figure} 
    We see that IQL outperforms AWAC on both environments, both in terms of raw performance and stability. 
\end{sol}
...
\begin{sol}
    We run RND with \texttt{total\_steps} set to 1000, 5000, 10000, and 20000 on the \texttt{PointmassMedium-v0} environment to produce a dataset for each variation. Then, after training CQL on each dataset, we obtain the following comparative results:
    \begin{figure}[!ht]\color{darkblue}
        \begin{center}
            \includegraphics[width=4in]{q4.3.PNG}
        \end{center}
        \caption{CQL on Datasets with Varying \texttt{total\_steps} for RND.}
    \end{figure}
    We see that CQL performs better on datasets produced with higher \texttt{total\_steps} values in RND. In particular, we observe flat returns for smaller values of \texttt{total\_steps}, and similar performance for $\texttt{total\_steps} \in \{10000, 20000\}$. 
\end{sol}
